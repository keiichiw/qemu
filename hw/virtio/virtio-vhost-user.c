...
static void virtio_vhost_user_cleanup_mem_table(VirtIOVhostUser *s)
{
    ...
    for (i = 0; i < VHOST_MEMORY_BASELINE_NREGIONS; i++) {
...
{
    uint32_t nregions = s->read_msg.payload.memory.nregions;
    ...
    for (i = 0; i < nregions; i++) {
        uint32_t mmap_offset = s->read_msg.payload.memory.regions[i].mmap_offset;
        uint32_t memory_size = s->read_msg.payload.memory.regions[i].memory_size;
        VirtIOVhostUserMemTableRegion *region = &s->mem_table[i];
        void *mmap_addr;

        region->total_size = mmap_offset + memory_size;
        if (region->total_size < mmap_offset ||
            region->total_size < memory_size) {
            goto err;
        }
        ...
        trace_virtio_vhost_user_memory_region(
            s,
            s->read_msg.payload.memory.regions[i].guest_phys_addr,
            s->read_msg.payload.memory.regions[i].memory_size,
            s->read_msg.payload.memory.regions[i].userspace_addr,
            s->read_msg.payload.memory.regions[i].mmap_offset,
            region->mmap_addr);
    }

    /* Export the mmapped vhost memory regions to the guest through PCI transport */
